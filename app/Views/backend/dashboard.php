<div class="row">
    <div class="col-sm-6 col-xl-3 text-center">
        <div class="card text-white bg-dark mb-3" style="max-width: 18rem;">
            <div class="card-body">
                <i class="fa fa-users fa-4x mb-3"></i>
                <h1 class="card-text"><?= $users ?></h1>
                <h2 class="card-text">Total Users</h2>
            </div>
        </div>
    </div>

    <div class="col-sm-6 col-xl-3 text-center">
        <div class="card text-white bg-dark mb-3" style="max-width: 18rem;">
            <a href="/admin/users/active_user">
                <div class="card-body">
                    <i class="fa fa-users fa-4x mb-3"></i>
                    <h1 class="card-text"><?= $connected ?></h1>
                    <h2 class="card-text">Total Connected</h2>
                </div>
            </a>
        </div>
    </div>

    <div class="col-sm-6 col-xl-3 text-center">
        <div class="card text-white bg-dark mb-3" style="max-width: 18rem;">
            <div class="card-body">
                <i class="fa fa-money fa-4x mb-3"></i>
                <h1 class="card-text">&#8377; <?= $payment ?></h1>
                <h2 class="card-text">Total Payment</h2>
            </div>
        </div>
    </div>
</div>